...
class FacebookManager:
    ...
    def generate_facebook_api_calls(self, target_calls: int = 50) -> Dict[str, Any]:
        """Generate Facebook Pages API calls for testing"""
        import time
        import random
        ...
        try:
            ...
            for i in range(target_calls):
                try:
                    ...
                    if endpoint_type == 0:
                        # Call pages endpoint
                        self._make_api_request("me/accounts")
                        call_breakdown["pages"] += 1
                        
                    elif endpoint_type == 1:
                        # Call insights endpoint
                        self._make_api_request(f"{page_id}/insights", {
                        ...
                        # Call posts endpoint
                        self._make_api_request(f"{page_id}/posts", {
                            "fields": "id,message,created_time",
                            "limit": 5
                        })
                        call_breakdown["posts"] += 1
                        
                    else:
                        # Call audience insights
                        self._make_api_request(f"{page_id}/insights", {
                    ...
                    logger.warning(f"Facebook API call failed: {call_error}")
                    failed_calls += 1
                    continue
                    
        except Exception as e:
            logger.error(f"Error generating Facebook API calls: {e}")
            raise
