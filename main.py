...
from chat.chat_manager_old import chat_manager
...
async def log_requests(request: Request, call_next):
    ...
    try:
        ...
        logger.error(f"   Processing time: {process_time:.2f}s")
        logger.error("=" * 80)
        raise
...
async def validation_exception_handler(request: Request, exc: RequestValidationError):
    ...
    # Get request body from exception object (body is already consumed by FastAPI)
    if hasattr(exc, 'body'):
        try:
            body_str = exc.body.decode('utf-8') if isinstance(exc.body, bytes) else str(exc.body)
            logger.error(f"📥 Request body: {body_str}")
        except Exception as e:
            logger.error(f"⚠️ Could not decode body: {e}")
            logger.error(f"📥 Raw body: {exc.body}")

    # Log all validation errors
    logger.error(f"❌ Validation errors ({len(exc.errors())} error(s)):")
    for i, error in enumerate(exc.errors(), 1):
        logger.error(f"   Error {i}:")
        logger.error(f"      Field/Location: {error.get('loc')}")
...
async def get_current_user(credentials: HTTPAuthorizationCredentials = Depends(security)) -> dict:
    """Get current authenticated user with auth token"""
    user_info = auth_manager.verify_jwt_token(credentials.credentials)
    # Add the actual JWT token to the user info
    user_info['auth_token'] = credentials.credentials
    return user_info
...
async def get_account_insights_summary(
    ...
    try:
        ...
        logger.info(f"🔍 ENDPOINT CALLED: /api/meta/ad-accounts/{account_id}/insights/summary")
        logger.info(f"🔍 ENDPOINT PARAMS: period={period}, start_date={start_date}, end_date={end_date}")
...
async def chat_message(
    ...
    try:
        logger.info("=" * 80)
        logger.info(f"📨 CHAT MESSAGE REQUEST RECEIVED")
        logger.info(f"👤 User: {current_user.get('email', 'unknown')}")
        logger.info(f"🔧 Module: {request.module_type}")
        logger.info(f"💬 Message: {request.message[:200]}{'...' if len(request.message) > 200 else ''}")
        ...
        logger.info(f"🔑 Auth Token Present: {'Yes' if current_user.get('auth_token') else 'No'}")
        logger.info("=" * 80)

        # 1️⃣ Generate response using chat_manager
        logger.info("🚀 Calling chat_manager.process_chat_request()")
        ...
        logger.info(f"🎯 Triggered Endpoint: {response.triggered_endpoint}")
        logger.info(f"📊 Endpoint Data Keys: {list(response.endpoint_data.keys()) if response.endpoint_data else None}")
        logger.info("=" * 80)
        ...
        logger.error("=" * 80)
        logger.error(f"❌ ERROR PROCESSING CHAT MESSAGE")
...
@app.get("/api/chat/continue/{session_id}")
async def continue_chat(
    session_id: str,
    user_response: str = Query(..., description="User's response or selection (JSON encoded)"),
    module_type: str = Query(..., description="Module type"),
    current_user: dict = Depends(get_current_user)
):
    """Continue a chat session after user provides input (Meta Ads selection flow)"""
    try:
        logger.info("=" * 80)
        logger.info(f"🔄 CONTINUE CHAT REQUEST RECEIVED")
        logger.info(f"👤 User: {current_user.get('email', 'unknown')}")
        ...
        logger.info("=" * 80)
        logger.info(f"✅ CONTINUE CHAT RESPONSE GENERATED")
        logger.info(f"🆔 Session ID: {response.session_id}")
        logger.info(f"📝 Response Length: {len(response.response)} chars")
        logger.info("=" * 80)
        ...
        logger.error("=" * 80)
        logger.error(f"❌ ERROR CONTINUING CHAT")
        logger.error(f"Error Type: {type(e).__name__}")
        logger.error(f"Error Message: {str(e)}")
        logger.error("=" * 80)
        logger.error(f"Full traceback:", exc_info=True)
...
async def get_conversation(
    ...
    try:
        logger.info(f"📖 Loading conversation: {session_id} for module: {module_type}")
        ...
        return {
            ...
            "module_type": module_type,
            "messages": messages,
            "created_at": conversation["created_at"].isoformat() if hasattr(conversation["created_at"], 'isoformat') else conversation["created_at"],
            "last_activity": conversation.get("last_activity", conversation["created_at"]).isoformat() if hasattr(conversation.get("last_activity", conversation["created_at"]), 'isoformat') else conversation.get("last_activity", conversation["created_at"])
        }
...
async def delete_chat_sessions(
    ...
    try:
        ...
        logger.error(f"❌ Error deleting chat sessions: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail=str(e))

@app.get("/chat/continue/{session_id}")
async def continue_chat_session(
    session_id: str,
    user_response: str = Query(...),
    module_type: str = Query(...),
    current_user: dict = Depends(get_current_user)
):
    """Continue a chat session after user provides selection"""
    try:
        import json
        
        # Parse user response (should be a list of IDs)
        selected_ids = json.loads(user_response)
        
        # Get auth token
        auth_token = current_user.get("token") or current_user.get("facebook_token")
        
        # Continue the chat
        from chat.graphs.graph_orchestrator import continue_chat
        final_state = await continue_chat(
            session_id=session_id,
            user_response=selected_ids,
        ...
        return ChatResponse(
            ...
            endpoint_data={
                ...
                "requires_selection": _check_if_needs_selection(final_state)
            }
        )
...
def _check_if_needs_selection(state: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    """Check if state requires user selection"""
    if state.get("needs_user_selection"):
        return {
            "type": state.get("selection_type"),
            "options": state.get("selection_options", []),
    ...
    try:
        ...
        for session in sessions:
            ...
            if 'last_activity' in session:
                session['last_activity'] = session['last_activity'].isoformat() if hasattr(session['last_activity'], 'isoformat') else str(session['last_activity'])
