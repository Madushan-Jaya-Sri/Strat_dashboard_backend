...
class MetaManager:
    ...
    def get_campaigns_with_totals(self, account_id: str, period: str = None, 
                                    ...
                                    max_workers: int = 2) -> Dict:  # CHANGED: Default to 2, not 5 or 10
            """
            Fetch campaigns with VERY LOW concurrency to avoid rate limits.
            Since you only have 4-5 campaigns, speed isn't critical.
            """
            if start_date and end_date:
                self._validate_date_range(start_date, end_date)
            ...
            # FORCE max_workers to be 2 or less
            max_workers = min(max_workers, 2)
            
            logger.info(f"Fetching campaigns with max_workers={max_workers} (rate limit safe mode)")
            ...
            try:
                ...
                # Step 2: Fetch insights with VERY LOW concurrency
                campaigns_data = []
                ...
                logger.info(f"Campaigns processed: {len(campaigns_data)}, with data: {campaigns_with_activity}")
    ...
    def get_adsets_by_campaigns(self, campaign_ids: List[str], period: str = None, 
                            start_date: str = None, end_date: str = None) -> List[Dict]:
        """
        Get ad sets for campaigns (4-5 campaigns).
        ...
        for idx, campaign_id in enumerate(campaign_ids):
            try:
                logger.info(f"[{idx+1}/{len(campaign_ids)}] Fetching ad sets for campaign: {campaign_id}")
                
                # ADD DELAY between each campaign
                if idx > 0:
                    time.sleep(0.5)  # 500ms delay between campaigns
                ...
                while next_url:
                    ...
                    if response.status_code == 200:
                        page_data = response.json()
                        adsets.extend(page_data.get('data', []))
                        next_url = page_data.get('paging', {}).get('next')
                    else:
                        break
